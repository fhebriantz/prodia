...
export default function Welcome(props) {
    ...
    const compressImagesToZip = async (imageUrls) => {
        const zip = new JSZip();
        ...
        const promises = list.map((imgUrl, index) => {
          return fetch(imgUrl)
            .then((response) => response.blob())
            .then((blob) => folder.file(`image_${index}.png`, blob));
        });
    ...
    const handleClick = async () => { 
        for (let i = 0; i < loop; i++) {
            ...
            await axios.post(`${url}/ci/generate`,data)
            .then(({ data }) => {
                setTimeout(function() {
                    axios.post(`${url}/ci/render/${data?.job}`)
                    .then(({ data }) => {
                        console.log(data)
                        ...
                        axios.post(`${url}/ci/insert`,databody)
                        .then(({ data }) => {
                            setListimage(data)
                        })
                    })
                    ...
                    <div className="flex">
                        ...
                        <div className='max-full p-2'>
                            ...
                            <Row>
                                ...
                                <Button className='m-1' type="primary" onClick={handleDownload} loading={loadings}>Download Image</Button>
                            </Card>
                        </div>
                        <div className='max-full p-2'>
                            <Row gutter={[8,8]}>
